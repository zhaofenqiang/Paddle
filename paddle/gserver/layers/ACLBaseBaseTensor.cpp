...
BaseBaseTensor::BaseBaseTensor(arm_compute::TensorInfo &info) noexcept
    : _target(TargetHint::DONT_CARE), _info(info),  _tensor(nullptr)
{
}

BaseBaseTensor::BaseBaseTensor(BaseBaseTensor &&src) noexcept
    : _target(src._target),
...
arm_compute::ITensor *BaseBaseTensor::set_target(TargetHint target)
{
    switch(target)
    {
#ifdef USE_OPENCL
...
SubTensor::SubTensor(BaseBaseTensor* parent, arm_compute::TensorShape& tensor_shape, arm_compute::Coordinates& coords) noexcept
