//conflict with paddle::baseTensor, so name this calss BaseBaseTensor
#pragma once
...
class BaseBaseTensor {
public:
    BaseBaseTensor(arm_compute::TensorInfo &info) noexcept;
    virtual ~BaseBaseTensor(){
    }
    BaseBaseTensor(BaseBaseTensor &&src) noexcept ;
...
{
public:
    SubTensor(BaseBaseTensor* parent, arm_compute::TensorShape& tensor_shape, arm_compute::Coordinates& coords)noexcept;
    ~SubTensor(){}
    arm_compute::ITensor       *tensor() ;
